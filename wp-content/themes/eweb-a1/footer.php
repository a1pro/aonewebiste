			...
			<div class="container">
				...
				<div class="col span_7">
					...
					<?php if(!empty($options['disable-auto-copyright']) && $options['disable-auto-copyright'] == 1) { ?>
						<p><?php if(!empty($options['footer-copyright-text'])) echo $options['footer-copyright-text']; ?> <a href="http://a1professionals.com/">WP themes</a></p>	
					<?php } else { ?>
						<p>&copy; <?php echo date('Y') . ' ' . get_bloginfo('name'); ?>. <?php if(!empty($options['footer-copyright-text'])) echo $options['footer-copyright-text']; ?> Made with <img id="gificon" src="http://192.168.1.253/a1-new/wp-content/uploads/2016/04/footer-icon.gif"> in Chandigarh, India.</p>
					<?php } ?>
